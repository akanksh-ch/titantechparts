...
// 1. Define validator files explicitly (listFiles is not reliable in all environments)
const fs = require('fs');
const validatorFiles = [
    { name: 'Validator-User.JSON', collection: 'User' },
    { name: 'Validator-Inventory.JSON', collection: 'Inventory' },
    { name: 'Validator-Orders.JSON', collection: 'Orders' }
];

print(`--- Processing ${validatorFiles.length} validator file(s) ---`);

// 2. Create collections with validators based on defined files
validatorFiles.forEach(f => {
    const collectionName = f.collection;
    const filePath = '/docker-entrypoint-initdb.d/' + f.name;
